    ...
    for run_index, run_name in enumerate(run_names):
        ...
        with open(f"./experiments/{model_name}/{run_name}/{run_name}_errors.txt", 'r') as f:
            ...
            for weight_index in range(no_of_weights):
                ...
                errors["train"][weight_index, run_index] = float(line[1])
                errors["val"][weight_index, run_index] = float(line[2])
                errors["test"][weight_index, run_index] = float(line[3])
                errors["ood"][weight_index, run_index] = float(line[4])
