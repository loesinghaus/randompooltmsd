...
for model_index, model_name in enumerate(model_names):
    ...
    with open(f"./errors/errors_{model_name}.txt", 'r') as f:
        ...
        for run_index in range(no_of_runs):
            ...
            if line[1] != 'nan':
                errors["train"][model_index, run_index] = float(line[1])
                errors["val"][model_index, run_index] = float(line[2])
                errors["test"][model_index, run_index] = float(line[3])
                errors["ood"][model_index, run_index] = float(line[4])
...
plt.savefig("./plots/errors_models_tanh_bar.svg", format='svg')
plt.close()
