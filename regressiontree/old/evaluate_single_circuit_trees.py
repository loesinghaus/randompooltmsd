...
for model_index, model_name in enumerate(model_names):
    ...
    with open(f"./experiments/SingleCircuit/{model_name}/{model_name}Single{run_num}/{model_name}Single{run_num}_best_errors.txt", 'r') as f:
        f.readline()
    ...
    # ----------- load tree ----------- 
    my_model, random_seed = pickle.load(open(f"./experiments/SingleCircuit/{model_name}/{model_name}Single{run_num}/"
    f"trees_pickle/{model_name}Single{run_num}_alpha_{best_error_index}.p", "rb"))
    ...
    if plot_train_test:
        ...
        ax.scatter(x_vals_val, sorted_prediction_val, color='g', alpha=size_params["alpha"], s=size_params["s"],linewidths=0)
        ax.plot(x_vals_train, sorted_y_train, linewidth=size_params["lw"], color='b', label=f"circuit one, mse={train_error:.2f}")
        ax.plot(x_vals_val, sorted_y_val, color='g', linewidth=size_params["lw"], label=f"circuit two, mse={val_error:.2f}")
    else:
...
plt.tight_layout()
if plot_train_test:
    plt.savefig(f"./plots/single_tree_eval_{run_num}_train_val.svg", format='svg')
else:
    plt.savefig(f"./plots/single_tree_eval_{run_num}_ood.svg", format='svg')
plt.close()
     
#output_df.to_excel(f"eval_Single_{run_num}.xlsx")