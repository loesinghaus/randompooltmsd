...
experiment_names = ["All","EnsembleFull", "Choice1", "Choice2",
...
for experiment_name in experiment_names:
    ...
    run_names = [f"{experiment_name}MixedR{i*10}" for i in range(1,11)]
    ...
    for run_index, run_name in enumerate(run_names):
        ...
        for dropout_index, dropout_p in enumerate(dropout_ps):
            ...
            my_model = MLP(input_dims=input_dims, dropout_p=dropout_p, weight_decay=weight_decay)
        ...
        with open(f"./experiments/{experiment_name}/{run_name}/{run_name}_errors.txt", 'w') as f:
            f.write("weight_decay,dropout,train_error,val_error,test_error,ood_error\n")
            for weight_index, weight in enumerate(weight_decays):
                f.write(f"{weight},{dropout_ps[weight_index]},{train_errors[weight_index]},{val_errors[weight_index]},"
    ...
    with open(f"./errors/errors_{experiment_name}.txt", 'w') as f:
        f.write("run,best_index,train_error,val_error,test_error,ood_error\n")
        for run_index, run_name in enumerate(run_names):
